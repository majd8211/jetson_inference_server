...
from model_code.pytorch_yolov3.torch_yolov3.evaluate import bbox_iou, bbox_wh_iou
from model_code.pytorch_yolov3.torch_yolov3.utils import to_cpu, parse_model_config
...
def build_targets(predict_boxes, predict_cls, target, anchors, ignore_thres):
    ...
    for i, anchor_ious in enumerate(ious.t()):
        noobj_mask[bb[i], anchor_ious > ignore_thres, gj[i], gi[i]] = 0

    # Coordinates
    tx[bb, best_n, gj, gi] = gx - gx.floor()
    ty[bb, best_n, gj, gi] = gy - gy.floor()
    # Width and height
    tw[bb, best_n, gj, gi] = torch.log(gw / anchors[best_n][:, 0] + 1e-16)
    th[bb, best_n, gj, gi] = torch.log(gh / anchors[best_n][:, 1] + 1e-16)
    # One-hot encoding of label
    tcls[bb, best_n, gj, gi, target_labels] = 1
    # Compute label correctness and iou at best anchor
    class_mask[bb, best_n, gj, gi] = (predict_cls[bb, best_n, gj, gi].argmax(-1) == target_labels).float()
    iou_scores[bb, best_n, gj, gi] = bbox_iou(predict_boxes[bb, best_n, gj, gi], target_boxes, x1y1x2y2=False)
